...
func (m *CreatureSpawnManager) RegisterSpawn(spawnID, creatureID string, x, y float64, z int, maxHP float64) (*CreatureSpawnState, error) {
	...
	state := &CreatureSpawnState{
		...
		DamageContributors: make(map[string]float64),
		Version:            1,
	}
...
// AddDamageContribution records authoritative damage contribution for one alive spawn.
func (m *CreatureSpawnManager) AddDamageContribution(spawnID, playerID string, damage float64) (*CreatureSpawnState, bool) {
	if spawnID == "" || playerID == "" || damage <= 0 {
		return nil, false
	}
...
func cloneCreatureSpawnState(state *CreatureSpawnState) *CreatureSpawnState {
	...
	copyState := *state
	if state.DamageContributors != nil {
		copyState.DamageContributors = make(map[string]float64, len(state.DamageContributors))
		for playerID, totalDamage := range state.DamageContributors {
			copyState.DamageContributors[playerID] = totalDamage
		}
	}
